...
// Start a test series named marketToken, it will use 10 test accounts
contract("MarketToken", async (accounts) => {
  // each it is a new test, and we name our first test initial supply
  it("initial supply", async () => {
    // wait until devtoken is deplyoed, store the results inside marketToken
    // the result is a client to the Smart contract api
    let marketToken = await MarketToken.deployed();
    // call our totalSUpply function
    let supply = await marketToken.totalSupply();
  ...
  it("minting", async () => {
    ...
    let intial_balance = await marketToken.balanceOf(accounts[1]);
    ...
    let totalSupply = await marketToken.totalSupply();
    await marketToken.mint(accounts[1], 100);
    // Grab the balance again to see what it is after calling mint
    let after_balance = await marketToken.balanceOf(accounts[1]);
    let after_supply = await marketToken.totalSupply();
    ...
    try {
      // Mint with address 0
      await marketToken.mint("0x0000000000000000000000000000000000000000", 100);
    } catch (error) {
      assert.equal(
        error.reason,
        "Grr, cannot mint to zero address",
  ...
  it("burning", async () => {
    ...
    let initial_balance = await marketToken.balanceOf(accounts[1]);

    // Burn to address 0
    try {
      await marketToken.burn("0x0000000000000000000000000000000000000000", 100);
    } catch (error) {
      assert.equal(
        error.reason,
        "Grr, cannot burn from zero address",
    ...
    try {
      await marketToken.burn(accounts[1], initial_balance + initial_balance);
    } catch (error) {
      assert.equal(
        error.reason,
        "Grr, cannot burn more than account owns",
    ...
    let totalSupply = await marketToken.totalSupply();
    try {
      await marketToken.burn(accounts[1], initial_balance - 50);
    } catch (error) {
      assert.fail(error);
    }

    let balance = await marketToken.balanceOf(accounts[1]);
    ...
    let newSupply = await marketToken.totalSupply();
  ...
  it("transfering tokens", async () => {
    devToken = await MarketToken.deployed();
    ...
    assert.equal(
      ...
      "Balance should have increased on reciever"
    );
    ...
    assert.equal(
      ...
      "Should have given accounts 2 20 tokens"
    );
    ...
    try {
      ...
      assert.equal(error.reason, "Grr, cant transfer more than account holds");
    }
  });
  ...
  it("allow account some allowance", async () => {
    ...
    try {
      ...
      assert.equal(
        error.reason,
        "Grr, approve cannot be done from zero address",
  ...
  it("transfering with allowance", async () => {
    ...
    try {
      ...
      assert.equal(
        error.reason,
        "You are not approved for spending this amount",
    ...
    try {
      // Account 1 should have 100 tokens by now to use on account 0
      // lets try using more
      let worked = await devToken.transferFrom(accounts[0], accounts[2], 50, {
        from: accounts[1],
      });
    } catch (error) {
      assert.fail(error);
